...
function init()
  ...
  params:add { type = 'binary', name = 'recording', id = 'recording', behavior = 'toggle', allow_pmap = true, action = function(
    v)
    toggle_recording(v)
  end
  }
  params:add { type = 'binary', name = 'clear buffer', id = 'clear', behavior = 'momentary', allow_pmap = true, action = function(
    v)
    if prevent_saveload then do return end end
    if v == 1 then
      print("clearing")
  ...
  for i = 1, 6 do
    ...
    voice[i].le = {
      set = state.buffer_size[state.buffer],
      adj = 0,
      calc = 0,
      lfo = 1,
      lfo_offset = math.random(0, 60),
  ...
  for i = 1, 6 do
    for j = 1, 6 do
      if j == 1 then
        if state.lfo_freeze == 0 then
          if params:get("quantize") == 1 then
            ...
            voice[i].level.lfo = math.abs(calculate_lfo(beat_sec * voice[i].level.lfo_period,
              beat_sec * voice[i].level.lfo_offset))
          end
        end
      ...
      elseif j == 3 then
        if state.lfo_freeze == 0 then
          if params:get("quantize") == 1 then
            ...
            voice[i].rate.lfo = math.abs(calculate_lfo(beat_sec * voice[i].rate.lfo_period,
              beat_sec * voice[i].rate.lfo_offset))
          end
        end
        voice[i].rate.calc = util.clamp(round(voice[i].rate.set * voice[i].rate.lfo + voice[i].rate.adj), 1,
          const_num_rates)
      elseif j == 4 then
        ...
        if state.lfo_freeze == 0 then
          if params:get("quantize") == 1 then
            ...
            voice[i].sign.lfo = 1 + calculate_lfo(beat_sec * voice[i].sign.lfo_period, beat_sec *
              voice[i].sign.lfo_offset)
          end
        end
      ...
      elseif j == 6 then
        if state.lfo_freeze == 0 then
          if params:get("quantize") == 1 then
            voice[i].le.lfo = calculate_lfo(voice[i].le.lfo_period, voice[i].le.lfo_offset) *
                state.buffer_size[state.buffer] / 2 + 2 * state.buffer_size[state.buffer] / 3
          else
            voice[i].le.lfo = calculate_lfo(beat_sec * voice[i].le.lfo_period, beat_sec * voice[i].le.lfo_offset) *
                state.buffer_size[state.buffer] / 2 + 2 * state.buffer_size[state.buffer] / 3
          end
        end
        voice[i].le.calc = util.clamp(voice[i].ls.calc + voice[i].le.lfo + voice[i].le.adj,
          state.buffer_size[state.buffer] / 8, state.buffer_size[state.buffer])
        softcut.loop_end(i, 1 + voice[i].le.calc)
      elseif j == 5 then
        if state.lfo_freeze == 0 then
          if params:get("quantize") == 1 then
            voice[i].ls.lfo = calculate_lfo(voice[i].ls.lfo_period, voice[i].ls.lfo_offset) *
                state.buffer_size[state.buffer] / 2 + state.buffer_size[state.buffer] / 3
          else
            voice[i].ls.lfo = calculate_lfo(voice[i].ls.lfo_period * beat_sec, voice[i].ls.lfo_offset * beat_sec) *
                state.buffer_size[state.buffer] / 2 + state.buffer_size[state.buffer] / 3
          end
        end
...
function redraw()
  ...
  for i = 1, 6 do
    ...
    j = j + 1
    draw_dot(j, p)
    screen.move(8 + state.shift * 3 +
      util.clamp(const_line_width * (voice[i].ls.calc) / state.buffer_size[state.buffer], 0, 110), p)
...
function setup_sharing(script_name)
  ...
  params:add { type = 'binary', name = 'refresh directory', id = 'share_refresh', behavior = 'momentary', action = function(
    v)
